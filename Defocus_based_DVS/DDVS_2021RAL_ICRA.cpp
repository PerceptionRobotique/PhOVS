...
//Pour images de 640x512
#define FACT 0.5
#define DI 5
#define DJ 5
#define NBRI (512*FACT-2*DI)
#define NBRJ (640*FACT-2*DJ)
//#define PAS (4*FACT)
...
*/
  ...
  //Parametres intrinseques pour FlirCam
	int larg = 640*FACT, haut = 512*FACT;
  double u0 = 320*FACT;
  double v0 = 256*FACT;
  // Fujinon lens
...
//  bool dofs[6] = {true, true, true, false, false, true};
...
std::cout << "ok 3" << std::endl;
  ...
  //argv[2] : tY
  //argv[2] : tZ
  //argv[2] : rX directly expected in degrees
  //argv[2] : rY directly expected in degrees
  //argv[2] : rZ directly expected in degrees
  ...
  if(argc > 2)
  {
    ...
    if(argc > 3)
    {
      ...
      if(argc > 4)
      {
        ...
        if(argc > 5)
        {
          p_init[3] = atoi(argv[5])/180.0;

          if(argc > 6)
          {
            p_init[4] = atoi(argv[6])/180.0;

            if(argc > 7)
            {
              p_init[5] = atoi(argv[7])/180.0;
            }
          }
        }
      }
