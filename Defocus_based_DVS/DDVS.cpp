...
//./DDVS 1000 200 100 15 1
// factor to meters
// radius of the sphere on which randomly thow initial positions
// threshold on Z below which to discard initial position 
// bound (degrees) of the rZ interval in which randomly throwing 
...
//  std::cout << "Deplacement vers pose initiale " << argc << " " << atof(argv[2])/atof(argv[1]) << " " << atof(argv[3])/atof(argv[1]) << std::endl;
  ...
  //argv[1] : metric scale
  //argv[2] : radius of the sphere on which randomly thow initial positions
  //argv[3] : threshold on Z below which to discard initial position 
  //argv[4] : bound (degrees) of the rZ interval in which randomly throwing 

  double radius = 10;
  double Zthreshold = 10;
  double rZ_range = 15;
  unsigned int nbExp = 1;

  if(argc > 2)
  {
    radius = atof(argv[2])/atof(argv[1]);

    if(argc > 3)
    {
      Zthreshold = atof(argv[3])/atof(argv[1]);

      if(argc > 4)
      {
        rZ_range = atoi(argv[4])*M_PI/180.0;
  ...
  int iExp = 0;
  for(iExp = 0 ; iExp < nbExp ; iExp++)
    rng();

  double az = 0, el = 0;
  double tX = 0, tY = 0, tZ = 10000;
  
  while( (tZ > Zthreshold) || (tY < -0.35) || (tY > 0.5) || (tZ < -0.5)) 
  {
/*
  //pour 600_1 a 600_9
    az = (((rng() - 0.5)*2*180.0))*M_PI/180.0;
    el = (((rng() - 0.5)*2*90.0))*M_PI/180.0;
*/
    az = ((int)(rng()*360))*M_PI/180.0;
    iExp++;
    el = ((int)(rng()*360))*M_PI/180.0;
    iExp++;
    tZ = radius*cos(el)*cos(az);
    tY = radius*sin(el);
  }
  std::cout << "iExp : " << iExp << std::endl;

  tX = radius*cos(el)*sin(az);
  tY = radius*sin(el);
  ...
  double rX = -atan2(dY, dZ);//+0.05;//-0.05;
  double rY = atan2(dX, dZ);//-0.05;

/*  vpColVector v1(3),v2(3), axis;
  v1[0] = v1[1] = 0;
  v1[2] = 1;
  ...
  axis = vpColVector::cross(v1,v2);

  std::cout << axis.t() << std::endl;
  axis.normalize();
  std::cout << axis.t() << std::endl;

  double angle = acos(vpColVector::dotProd(v1,v2));
...
/*	p_init[0] = 0.0;
  ...
  std::cout << "init pose reached" << std::endl;

  //Acquisition
  grabber.getFrame(Iacq);
  std::cout << "captured" << std::endl;
  vpImageTools::resize(Iacq, I, larg, haut, vpImageTools::vpImageInterpolationType::INTERPOLATION_CUBIC);//INTERPOLATION_LINEAR);//
  std::cout << "resized " << I.getWidth() << " " << I.getHeight() << std::endl;
  ...
  vpDisplay::getClick(I, btn);


  if(btn == vpMouseButton::button3)
    return -1;

  //  save validate random initial pose
  s.str("");
  s.setf(std::ios::right, std::ios::adjustfield);
  s << "convStud_initPoses.txt";
  filename = s.str();
  std::ofstream ficInitPoses(filename.c_str(), ios_base::app);
  ficInitPoses << p_init.t() << std::endl;
  ficInitPoses.close();
...
//  }
    ...
    //lambda   = 2.0;//0.5;//Fmin ; // 6 ddl //Marylin
